...
//	public static final int MSG_BLE_INIT_FAILED = MSG_BLE_NOT_ENABLED+1;
     ...
     * 指令代码：发送超时60s的指令
     */
    public static final int INDEX_SEND_DATA_TIMEOUT_SIXTY_SECONDS = INDEX_GET_MODEL + 1;
    ...
    public void setState(int state) {
        ...
        if (mConnectCallback!=null){
            if (state==0||state==3){
                mConnectCallback.setConnectStatus(false);
            }else if (state==2||state==1){
                mConnectCallback.setConnectStatus(true);
            }
        }
    ...
    public void setonConnectCallbackListener(ConnectCallback connectCallback){
    ...
    public byte[] open(final String address, final long timeOut, byte[] scanInfo)
            throws TimeoutException {
        setCurrentDeviceMac(address);
        ...
        FutureTask<byte[]> future =
                new FutureTask<>(new Callable<byte[]>() {//使用Callable接口作为构造参数
                    public byte[] call() {
                        try {
                            boolean connect = mBlEWapper.connect(address,timeOut);
//                            connect_mac(address);
                            if (connect) {
                                finalResult[0] = 0x00;
                            } else {
...
//				}
     ...
     * @param mac
     */
    public void connect_mac(final String mac) throws TimeoutException {
...
//					}
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            Log.e(TAG + "on", "intent.getAction():  " + intent.getAction() + "------------------------BLEreceiveCnt:  " + (BLEreceiveCnt++));
            ...
            if (BLEWapper.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                OwnLog.e(TAG, ".....................connected(接收到连接成功的广播)......................");
...
//   }
    ...
    public void send_dataTimeoutWithTimes(Context context, byte[] data, int timeSeconds) {
        OwnLog.i(TAG, "..................send_dataTimeoutWithTimes Thread........................" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
        runIndexProess(context, INDEX_SEND_DATA_TIMEOUT_SIXTY_SECONDS, data, timeSeconds);
    }
    ...
    public byte[] powerOn(long timeOut) throws TimeoutException,ConnectException {
        byte[] result = new byte[]{};
        ExecutorService executor = Executors.newSingleThreadExecutor();
        FutureTask<byte[]> futureTask = new FutureTask<>(new Callable<byte[]>() {
                @Override
                public byte[] call() throws ConnectException {
                    try {
                        return mLepaoProtocalImpl.powerOn();
                    } catch (LPException e) {
                        e.printStackTrace();
    ...
    public boolean powerOff(){
        boolean b = false ;
        try {
             b = mLepaoProtocalImpl.closeSmartCard();
        } catch (LPException e) {
            e.printStackTrace();
        } catch (BLException e) {
            e.printStackTrace();
    ...
    public byte[] exchangeWithData(final byte[] data, long timeOut) throws TimeoutException, ConnectException, TransportException {
        ExecutorService executor = Executors.newSingleThreadExecutor();
        FutureTask<byte[]> futureTask = new FutureTask<>(new Callable<byte[]>() {
            @Override
        ...
        try {
             bytes = futureTask.get(timeOut, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
    ...
    public void runIndexProess(Context context, int index, byte[] date, int timeSeconds) {

        synchronized (deviceAddressQueue) {
            ProessThread proessThread = new ProessThread(context, index, date, timeSeconds);
...
//		}
    ...
    private class ProessThread extends Thread {
        ...
        public ProessThread(Context context, int index, byte[] title, int timeSeconds) {
            mContext = context;
            this.index = index;
            this.title = title;
...
//				    	msg = mHandler.obtainMessage(INDEX_SET_DEVICE_TIME);
                    ...
                    case INDEX_SEND_DATA:
                        ...
                        msg.sendToTarget();
                        break;
                    case INDEX_SEND_DATA_TIMEOUT_SIXTY_SECONDS:
                        OwnLog.d(TAG, ".................INDEX_SEND_DATA................");
                        msg = mHandler.obtainMessage();
                        msg.what = MSG_BLE_DATA;
                        msg.arg1 = INDEX_SEND_DATA_TIMEOUT_SIXTY_SECONDS;
                        msg.obj = mLepaoProtocalImpl.sendDataBleTimeout(title, timeSeconds);
                        msg.sendToTarget();
                        break;
