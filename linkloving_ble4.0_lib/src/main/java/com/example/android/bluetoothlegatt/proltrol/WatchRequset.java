...
public class WatchRequset {
	...
	public static final int  DATA_LEN = 1024;  // 命令数据长度限制
	...
	public  WatchRequset(int length) {
		if( length + 6 > 255){//长度过长
			...
			Log.e("leng++","(byte)( (data.length >> 8) & 0x00FF )+      "+(byte)( (data.length >> 8) & 0x00FF ));
		}else{
	...
	public  WatchRequset  makeCheckSum() {
		...
		else
		{
			...
			len = len +2 ;
			data[1]=(byte) len;
			Log.e("open","Len-------"+(len>(byte)0xff)+"------"+len+"--------"+(byte)0xff);
			if (len>255){
				Log.e("open","Len-------"+(len>(byte)0xff)+"------"+len+"--------"+(byte)0xff);
				data[2]|=(byte)0x01;
			}
			...
			for( int i = 0; i < data.length; ++i )
				sum += ( data[i] & 0xFF );
			data[len-2] = (byte)( (sum >> 8) & 0x00FF );
			data[len-1] = (byte)( sum & 0x00FF );
		}
