...
public class LepaoProtocalImpl implements LepaoProtocol {
	...
	public byte[] sendDataBleTimeout(byte[] data,int timeSeconds){
		WatchRequset req = new WatchRequset();
		WatchResponse watchResponse = null ;
		try {
			 req.appendByte((byte) (seq += 2)).appendByte(data).makeCheckSum();
			watchResponse = sendData2BLEWithTime(req,timeSeconds);
		} catch (LPException e) {
			e.printStackTrace();
		} catch (BLException e) {
			e.printStackTrace();
	 ...
	 * 现用的发送方法60s
	 * */
	public synchronized WatchResponse sendData2BLEWithTime(WatchRequset req, int timeSeconds) throws BLException, LPException {

		try {
			WatchResponse resp = sendData2BLEImplTimeout60s(req,timeSeconds);
			retryTimes = 0;
			return resp;
		} catch (BLException e) {
			OwnLog.e("KILL", e.getMessage());
...
// 发送60秒的数据
	private WatchResponse sendData2BLEImplTimeout60s(WatchRequset req,int timeSeconds) throws LPException, BLException {
		try {
			byte[] data = req.getData();
			//支持95命令把packageheader最后一为置成01;
			Log.e("opencard",data.length+"-------------------");
			byte[] resp = BLEWapper.getInstence().send60s(data,0,timeSeconds);
			retryTimes = 0;
			LPUtil.printData(resp,"sendData2BLEImplTimeout60s的response是");
			return new WatchResponse(resp, 0, resp.length);
		} catch (LPException e) {
			OwnLog.e(TAG, "retry:" + retryTimes);
			if (retryTimes++ > MAX_RETRY_TIMES) throw e;
			OwnLog.e(TAG,"LPException e"+e.toString());
			try {
				Thread.sleep(300);
			} catch (InterruptedException e1) {
				e1.printStackTrace();
			}
			return sendData2BLEImplTimeout60s(req,timeSeconds);
		} catch (BLException e) {
			OwnLog.e(TAG, "retry:"+retryTimes);
			if(retryTimes++>MAX_RETRY_TIMES) throw e;
	...
	private WatchResponse sendData2BLEImpl(WatchRequset req) throws LPException, BLException {
		try {
			...
			//支持95命令把packageheader最后一为置成01;
			Log.e("opencard",data.length+"-------------------");
			byte[] resp = BLEWapper.getInstence().send(data,0);
			retryTimes = 0;
			return new WatchResponse(resp, 0, resp.length);
		} catch (LPException e) {
			if (retryTimes++ > MAX_RETRY_TIMES) throw e;
	...
	// 获取设备信息
	@Override
	public LPDeviceInfo getAllDeviceInfoNew(String userId) throws BLException, LPException {
//		WatchRequset req = new WatchRequset();
//		byte ff = (byte) 0xff;
//		byte _00=(byte) 0x00;
//		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_ALL_USER_INFO).appendByte(ff).makeCheckSum();
...
//		}
		...
		byte ff = (byte) 0xff;
		byte _00=(byte) 0x00;
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_ALL_USER_INFO).appendByte(ff)
				.appendInt(0)
				.makeCheckSum();
		LPUtil.printData(req.getData(), " getAllDeviceInfoNew");
		WatchResponse resp = this.sendData2BLE(req);
	...
	public LPDeviceInfo getModelName() throws BLException, LPException {
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_DEVICEINFO).makeCheckSum();
	...
	public int registerNew(LPUser userInfo) throws BLException,
		...
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_REGISTER)
				.appendByte((byte) 0)
...
//				.appendInt((int)(System.currentTimeMillis() / 1000))
	...
	public List<LPSportData> getSportDataNew(int offset, int length, int detial)
		...
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_SPORT_RECORDER).appendByte((byte) offset)
	...
	public boolean formatDevice() throws BLException, LPException {
		...
		byte format =0;
		byte format_test =1;
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_FORMAT_DEVICE).appendByte(format).makeCheckSum();
	...
	public int getOADHeadBack(byte[] data) throws BLException, LPException {
		...
		LPUtil.printData(data, "SEND OADHEAD");
		byte[] resp = this.sendOADCommand(data, BLEWapper.OAD_HEAD, true);
		LPUtil.printData(resp, "接收 OADHEAD");
		if (resp[0] == 0x10) {
...
//				OwnLog.i(TAG, "重新发OAD块了:"+index);
	...
	public int setClock(LPDeviceInfo deviceInfo) throws BLException,
		...
		// COMMAND_SET_ALARM
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_SET_ALARM)
	...
	public LPDeviceInfo setSportTarget(LPDeviceInfo deviceInfo)
			throws BLException, LPException {
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_SET_TASK)
	...
	public int setPower(LPDeviceInfo deviceInfo)throws BLException, LPException {
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_SET_POWER_DECICE)
	...
	public String get_cardnum()throws BLException, LPException {
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_CARD_NUMBER).makeCheckSum();
	...
	public int set_name(LPDeviceInfo deviceInfo)throws BLException, LPException {
		...
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_SET_NAME).appendByte(name_len);
	...
	public void setBandRing() throws BLException,LPException {
		// COMMAND_SET_ALARM
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_BAND_RING).makeCheckSum();
	...
	public int setNotification(byte[] data) throws BLException,LPException {
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_SETNOTIFICATION).appendByte(data[0]).appendByte(data[1]).makeCheckSum();
	...
	public void keepstate() throws BLException,LPException {
		WatchRequset req = new WatchRequset();
		byte keepstate = 0;
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_DEVICE_ID).appendByte(keepstate).makeCheckSum();
	...
	public int requestbound() throws BLException, LPException {
		...
		byte peidui=0;  //手表
		byte test=1;    //手环
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_REQUEST_BOUND).appendByte(peidui).makeCheckSum();
...
//		flash_head=oadmakeCheckSum(9 ,141*4096,flash_head);            //hzk12地址 
	...
	// **********************for 一卡通相关的代码 START
	/**检查设备是否产生金额数据*/
	public boolean checkCardState() throws LPException, BLException
	{
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_CARD_STATE).makeCheckSum();
		LPUtil.printData(req.getData()," checkCardState()");
		WatchResponse resp = this.sendData2BLE(req);
		return resp.toOpenSmartCardOK(LepaoCommand.COMMAND_GET_CARD_STATE, "checkCardState");
	}

	/**已经检查完毕*/
	public boolean updateCardState() throws LPException, BLException
	{
		WatchRequset req = new WatchRequset();
		byte open =1;
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_GET_CARD_STATE).appendByte(open).makeCheckSum();
		LPUtil.printData(req.getData()," updateCardState()");
		WatchResponse resp = this.sendData2BLE(req);
		return resp.toOpenSmartCardOK(LepaoCommand.COMMAND_GET_CARD_STATE, "updateCardState");
	}
	...
	{
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD);
	...
	{
		WatchRequset req = new WatchRequset();
				req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD).appendByte(Card_Command.AID_STEP1).makeCheckSum();
	...
	{
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD).appendByte(Card_Command.AID_STEP2).makeCheckSum();
	...
	public boolean checkPin(LPDeviceInfo deviceInfo) throws LPException, BLException
	{
		...
		{
			WatchRequset req = new WatchRequset();
			req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD).appendByte(Card_Command.SUZHOU_CHECKPIN).makeCheckSum();
...
//				 if(deviceInfo.customer.equals(LPDeviceInfo.MIANYANG_XJ) || deviceInfo.customer.equals(LPDeviceInfo.MIANYANG_JIEJI) || deviceInfo.customer.equals(LPDeviceInfo.MIANYANG_DAIJI)){
    ...
    {  
				 WatchRequset req = new WatchRequset();
				 req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD);//00A6 0000 03
				 {   //00 B2 01 5C 00
					 //00 A6 00 00 03
			         req.appendByte((byte) 0x00)
	...
	{
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD).appendByte(Card_Command.YUNNAN_FUDIAN_CLOSE).makeCheckSum();
		if(openSmartCard()){
	...
	{
		WatchRequset req = new WatchRequset();
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_AID_CARD).appendByte(Card_Command.YUNNAN_FUDIAN_OPEN).makeCheckSum();
		if(openSmartCard()){
	...
	public boolean closeSmartCard() throws LPException, BLException
	{
		WatchRequset req = new WatchRequset();
		byte close =0;
		req.appendByte((byte)(seq+=2)).appendByte(LepaoCommand.COMMAND_CONTROL_CARD).appendByte(close).makeCheckSum();
...
//			return resp;
	...
	public byte[] senddata(byte[] data) throws BLException,LPException {
        ...
        //出现异常 重新组装并且发送
        if(resp_data.length == 4 && resp_data[2] ==(byte)0x61 && resp_data[1]==(byte)0x00)
        {
            byte[] dateNew = {0x00,(byte) 0xc0,0x00,0x00,0x00,0x00};
			dateNew[5] = resp_data[3];//sw2
			WatchRequset watchRequset = new WatchRequset();
			watchRequset
					.appendByte((byte)(seq+=2))
					.appendByte((byte) 0x91)
					.appendByte(dateNew)
					.makeCheckSum();
            byte[] resp_new = this.sendCommandNew(watchRequset.getData(), BLEWapper.NOT_OAD, false);
