...
//			LPUtil.printData(data1,"Revice22222");
...
//				LPUtil.printData(data,"Revice1111");
...
//        	return false;
	 ...
	 * 发送数据一起发
	 */
	public byte[] send2(byte[] boundData)throws  Exception{
		writeCharacteristic.setValue(boundData);
		...
		LPUtil.printData(data,"发送的数组是:");
		 if(data.length>20){
			 ...
			 for(int i=0;i<(data.length+19)/20;i++){
				...
				LPUtil.printData(send_data,"opencard+++++++发送的数组");
				 if(REC_CMD==0){
					 writeCharacteristic.setValue(send_data);
				 Log.e("opencard","opencard"+i);
				     mBluetoothGatt.writeCharacteristic(writeCharacteristic);
				     synchronized (BLEWapper.class) {
							try {
...
//		LPUtil.printData(tempData,"Revice  DataAll");
		...
		LPUtil.printData(revData,"发出命令接收到的数组是:");
		...
		reciveDataLen = -1;
//		if (((revData[2]&0x01)==(byte)0)&&(revData[3]==(byte)0x95)&&(revData[4]==(byte)0x00))
//		{
//			reciveDataLen=0x100 + revData[1];
//		}
	 ...
	 * 发送数据
	 */
	public byte[] send60s(byte[] data,int status,int timeSeconds) throws BLException, BLESendTimeOutException {
		if (mBluetoothAdapter == null || mBluetoothManager == null
				|| !mBluetoothAdapter.isEnabled()) {
			throw new BLException(BLErrCode.BLE_INIT_ERR);
		}
		...
		sendBusy = true;
		LPUtil.printData(data,"send60s发送的数据");
		if(data.length>20){
			...
			for(int i=0;i<(data.length+19)/20;i++){
				...
				if(REC_CMD==0){
					...
					synchronized (BLEWapper.class) {
						try {
							OwnLog.d(TAG, "...................waiting.........................");
							BLEWapper.class.wait(timeSeconds*1000);
						} catch (InterruptedException e) {
							OwnLog.d(TAG, "................InterruptedException................");
							e.printStackTrace();
						}
...
//
		...
		else
		{
			Log.e(TAG,"data.length<20");
			LENTHMORE20 = false;
			OwnLog.e(TAG, writeCharacteristic+"writeCharacteristic发送的数据是"+data);
			writeCharacteristic.setValue(data);
			boolean b = mBluetoothGatt.writeCharacteristic(writeCharacteristic);
			Log.e(TAG,b+"+++++++++++++data.length<20");
		}
		Log.e(TAG,"synchronizedTop");
		synchronized (BLEWapper.class) {
			try {
				Log.e(TAG,"synchronizedTop");
				OwnLog.d(TAG, "...................waiting.........................");
		...
		BLESendTimeOutException=false;
		sendBusy = false;
		Log.e(TAG,"reciveDataLen <= 0"+reciveDataLen);
		if (reciveDataLen <= 0) {
			CmdFinder.getInstence(new OnCmdFindedListener() {
		...
		//[1]:源数组； [2]:源数组要复制的起始位置； [3]:目的数组； [4]:目的数组放置的起始位置； [5]:复制的长度。 注意：[1] and [3]都必须是同类型或者可以进行转换类型的数组
		System.arraycopy(tempData, 0, revData, 0, reciveDataLen);
		Log.e(TAG, "!checksum(revData)");
		if(!checksum(revData)){
			Log.e(TAG, "进!checksum(revData)了");
			CmdFinder.getInstence(new OnCmdFindedListener() {
